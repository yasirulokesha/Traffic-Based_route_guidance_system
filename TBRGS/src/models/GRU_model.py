...
def gru_prediction_engine(data_set, site_id, target_time):
    
    model_path = f"TBRGS/src/models/GRU_model/models/gru_model_site_{int(site_id)}.keras"
    scaler_path = f"TBRGS/src/models/GRU_model/scalers/scaler_site_{int(site_id)}.save"
    ...
    try:
        ...
        try:
            ...
            predicted_value = float(predicted_volumes[-1, 0])
        except Exception as e:
...
def GRU_prediction(site_id, input_time):
    site_id = site_id
    ...
    predictions = gru_prediction_engine(data_set, site_id, input_time)
    return predictions
